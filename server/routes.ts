...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/fix-embeddings", async (req, res) => {
    try {
      ...
      for (const file of files) {
        try {
          console.log(
            `Regenerating embeddings for file ${file.id}: ${file.filename}`
          );
          ...
          console.error(
            `Error regenerating embeddings for file ${file.id}:`,
            error
          );
        }
      }
      ...
      res.json({
        success: true,
        message: `Regenerated embeddings for ${files.length} files`,
      });
    } catch (error) {
  ...
  app.post("/api/search", async (req, res) => {
    try {
      ...
      console.log("LOGGING the Query vector:", queryVector);
